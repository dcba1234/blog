...
export class CountryComponent implements OnInit {
  ...
  loadData() {
    this.svc.makeGet(`${API.COUNTRY}`, { ...this.form.value }).subscribe((data: any) => {
  ...
  active(type: 'active' | 'deactive', id) {
    this.svc.makeDelete(`${API.COUNTRY}/${type}/${id}`).subscribe(() => {
      this.loadData();
    })
  ...
  pageChange(page) {
    this.svc.makeGet(`${API.COUNTRY}`, { page: page.page }).subscribe((data: any) => {
      this.dataSource = data;
    })
  }
  ...
  submit() {
    ...
    this.closeModal();
    this.formSave.reset();
    this.svc.makePost(`${API.COUNTRY}/save`, saveItem).subscribe((data: any) => {
      this.toastr.success(
