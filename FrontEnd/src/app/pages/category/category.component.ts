...
export class CategoryComponent implements OnInit {
  ...
  loadData() {
    this.svc.makeGet(`${API.CATEGORY}`, { ...this.form.value }).subscribe((data: any) => {
  ...
  active(type: 'active' | 'deactive', id) {
    this.svc.makeDelete(`${API.CATEGORY}/${type}/${id}`).subscribe(() => {
      this.loadData();
    })
  ...
  pageChange(page) {
    this.svc.makeGet(`${API.CATEGORY}`, { page: page.page }).subscribe((data: any) => {
      this.dataSource = data;
    })
  }
  ...
  submit() {
    ...
    this.closeModal();
    this.formSave.reset();
    this.svc.makePost(`${API.CATEGORY}/save`, saveItem).subscribe((data: any) => {
      this.toastr.success(
