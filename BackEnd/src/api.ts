...
export const initAPIs = (app) => {
  try {
    ...
    router.post("/job", settingJob.save());

    router.get("/test", post.testCronJob());
    router.get("/test-job", post.getCronJob());

    router.post("/start-job", post.runJob());
    router.get("/get-job", post.getCronJob());
    router.post("/stop-job", post.stopJob());
