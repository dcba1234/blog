...
class InteractionController {
  ...
  colSave = ['Is_Active', 'Post_Slug' ,'Reason_Report_Id', 'Reason_Report_Other' , ...DEFAULT_COL]
  ...
  getReportReason() {
    return async (req, res) => { 
      const dt: [any, ISettingWeb[]] = await sqlHelper.select({columns: [{
        id: 'Reason',
      },], orderQuery: ' Reason DESC ' ,tablesName: 'reason_report', additionQuery: ` where ${this.tablesName}.Is_Active = 1 `});
      Promise.all(dt[1].map(async (i) => {
        i.modifiedBy = await sqlHelper.getUser(i.modifiedBy)[0];
        i.createdBy = await sqlHelper.getUser(i.modifiedBy)[0];
      }))
  ...
  save(isRP?) {
    return async (req, res) => { 
      ...
      if(isRP) {
        dataSave.Is_Report = true;
        dataSave.Reason_Report_Other = req.body.reasonReportOther;
        dataSave.Reason_Report_Id =  req.body.reasonReportId;
      }
      if(id) {
