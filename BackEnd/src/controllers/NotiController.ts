  ...
  getPage() {//  additionQuery: ` where ${this.tablesName}.Is_Active = true`,
    return async (req, res) => { 
      ...
      if(req.query.key) {
        ...
        key += `or ${this.tablesName}.Link_To like ${db.escape('%'+req.query.key.trim()+'%')} `
        key += `or ${this.tablesName}.Content like ${db.escape('%'+req.query.key.trim()+'%')} `
...
export const noti = new NotificationController();
