...
class UserController {
  ...
  getAll() {
    return async (req, res) => {
      const dt: [any, ISettingWeb[]] = await sqlHelper.select({
        ...
        additionQuery: ` where ${this.tablesName}.Is_Active = 1 `,
      });
      Promise.all(
        dt[1].map(async (i) => {
  ...
  getPage() {
    //  additionQuery: ` where ${this.tablesName}.Is_Active = true`,
    return async (req, res) => {
      let key = ` Role <> 'Admin' `;
      if (req.query.key) {
        key = `and ( ${this.tablesName}.Account like ${db.escape(
          "%" + req.query.key.trim() + "%"
        )} `;
        key += `or ${this.tablesName}.Email like ${db.escape(
          "%" + req.query.key.trim() + "%"
        )} `;
        key += `or ${this.tablesName}.First_Name like ${db.escape(
          "%" + req.query.key.trim() + "%"
        )} `;
        key += `or ${this.tablesName}.Last_Name like ${db.escape(
          "%" + req.query.key.trim() + "%"
        )} `;
        key += `or ${this.tablesName}.Created = ${db.escape(
          req.query.key.trim()
        )} ) `;
  ...
  updateInfo() {
    return async (req, res) => {
      ...
      dataSave.id = decoded.data.account;
      dataSave.Modified = moment().format("YYYY-MM-DD hh:mm:ss");
      dataSave.Created = moment().format("YYYY-MM-DD hh:mm:ss");
      dataSave.Created_By = await getUser(req);
      dataSave.Modified_By = await getUser(req);

      const rs = await sqlHelper.save(this.tablesName, dataSave, decoded.data.account);
      res.json(dataSave);
    };
  }
  ...
  uploadAvatar() {
    return async (req, res) => {
      const path = 'avatar/' + req.files.file.path.split('\\')[1];
      const decoded = await verifyToken(
        req.headers.authtoken,
        refreshTokenSecret
      );
  ...
  login() {
    return async (req, res) => {
      try {
        let result = (
          await sqlHelper.getUserValidate(
  ...
  getMyInfo() {
    return async (req, res) => {
      try {
        const account = (await getUser(req, true))[0];
